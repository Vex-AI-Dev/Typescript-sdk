#!/usr/bin/env npx tsx
/**
 * Live smoke test — exercises the PUBLISHED @vex_dev/sdk npm package
 * against the live API.
 *
 * Mirrors sdk/typescript/scripts/test_live_smoke.ts but imports from
 * the npm package instead of local source.
 *
 * Usage:
 *   cd sdk/typescript/scripts/npm-live-test
 *   npm install
 *   VEX_API_KEY=ag_live_... npm test
 */

import { Vex, VexBlockError, Session } from '@vex_dev/sdk';
import type { VexResult, VexConfigInput } from '@vex_dev/sdk';
...
async function scenario6MultiturnSession(): Promise<ScenarioResult> {
  ...
  const vex = makeVex({ mode: 'sync' });
  const session = new Session(vex, 'smoke-test-ts-npm-session');
...
async function main(): Promise<number> {
  console.log(`${BOLD}Vex TypeScript SDK (npm) Live Smoke Test${RESET}`);
  console.log(`  Package:  @vex_dev/sdk (from npm)`);
